...
const config = require("./shinji-config.json");
...
var configValue;
try {
    global.client.configPath = join(global.client.mainPath, "shinji-config.json");
    configValue = require(global.client.configPath);
    logger.loader("Found shinji-config.json file!");
} catch (e) {
    return logger.loader('"shinji-config.json" file not found."', "error");
}

try {
    for (const key in configValue) global.config[key] = configValue[key];
    logger.loader("shinji-config Loaded!");
} catch (e) {
    return logger.loader("Can't load file shinji-config!", "error")
}
...
try {
    var appStateFile = resolve(join(global.client.mainPath, config.APPSTATEPATH || "shinjistate.json"));
            ...
            for (const command of listCommand) {
                try {
                    ...
                    if (!config?.commandCategory) {
                        console.log(chalk.red(`•⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯\n『 COMMAND 』» ${chalk.hex("#7FFF00")(command)} Module is not in the correct format!`));
                        continue;
                    }
                    if (global.client.commands.has(config.name || '')) {
                        console.log(chalk.red(`•⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯\n『 COMMAND 』» ${chalk.hex("#7FFF00")(command)} Module is already loaded!`));
                        continue;
                    }
                    ...
                    if (dependencies) {
                        Object.entries(dependencies).forEach(([reqDependency, dependencyVersion]) => {
                            ...
                            try {
                                ...
                                global.loading.err(chalk.hex('#7FFF00')(errorMessage), '•⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯\n『 LOADED 』»');
                            }
                        });
                    }
                    ...
                    if (envConfig) {
                        ...
                        var configPath = require('./shinji-config.json');
                    ...
                    global.loading(`${chalk.hex('#7FFF00')(`LOADED`)} ${chalk.hex("#7FFF00")(config.name)} success`, `•⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯\n『 COMMAND 』»`);
                } catch (error) {
                    global.loading.err(`${chalk.hex('#7FFF00')(`LOADED`)} ${chalk.hex("#FFFF00")(command)} fail `, `•⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯\n『 COMMAND 』»`);
                }
            }
        })(),
        (async () => {
            ...
            for (const ev of events) {
                try {
                    ...
                    if (!config || !config.name || !run) {
                        global.loading.err(`${chalk.hex('#ff7100')(` LOADED `)} ${chalk.hex("#FFFF00")(ev)} Module is not in the correct format. `, "•⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯\n『 EVENT 』»");
                        continue;
                    }
                    if (global.client.events.has(config.name)) {
                        global.loading.err(`${chalk.hex('#ff7100')(` LOADED `)} ${chalk.hex("#FFFF00")(ev)} Module is already loaded!`, "•⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯\n『 EVENT 』»");
                        continue;
                    }
                    ...
                    global.loading(`${chalk.hex('#ff7100')(`LOADED`)} ${chalk.hex("#FFFF00")(config.name)} loaded successfully`, "•⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯\n『 EVENT 』»");
                } catch (err) {
                    global.loading.err(`${chalk.hex('#ff7100')(`EVENT`)} ${chalk.hex("#FFFF00")(command)} fail `, "•⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯\n『 LOADED 』»");
                }
            }
        })();
        console.log(cheerful(`•⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯\n『 BOT START 』»`));
        global.loading(`${chalk.hex('#ff7100')(`SUCCESS`)} Loaded ${global.client.commands.size} commands and ${global.client.events.size} events successfully`, `•⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯\n『 LOADED 』»`);
        global.loading(`${chalk.hex('#ff7100')(`TIME START`)} Launch time: ${((Date.now() - global.client.timeStart) / 1000).toFixed()}s`, `•⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯\n『 LOADED 』»`);
        const listener = require('./includes/listen')({ api: loginApiData });
        global.custom = require('./shinji-custom')({ api: loginApiData });
...
(async () => {
    try {
        console.log(sky(`•⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯\n『 DATABASE 』»`));
        global.loading(`${chalk.hex('#ff7100')(`[ CONNECT ]`)} Connected to JSON database successfully!`, `•⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯\n『 DATABASE 』»`);
        onBot();
    } catch (error) {
        global.loading.err(`${chalk.hex('#ff7100')('•⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯\n『 CONNECT 』»')} Cannot connect to the JSON database.`, "DATABASE");
    }
})();
