...
function startBot(message) {
    ...
    const child = spawn("node", ["--trace-warnings", "--async-stack-traces", "shinji.js"], {
        cwd: __dirname,
        stdio: "inherit",
        shell: true
    });
...
axios.get("https://raw.githubusercontent.com/Ber906/berber/main/package.json").then((res) => {
  logger(res['data']['name'], "[ NAME ]");
  logger("Version: " + res['data']['version'], "[ YUJIRU-V1 ]");
  logger(res['data']['credits'], "[ CREDITS ]");
});
startBot();
// THIZ BOT WAS MADE BY ME(CATALIZCS) AND MY BROTHER SPERMLORD - DO NOT STEAL MY CODE (つ ͡ ° ͜ʖ ͡° )つ ✄ ╰⋃╯
app.get('/', (req, res) => res.sendFile(__dirname+'/includes/cover/index.html'))
